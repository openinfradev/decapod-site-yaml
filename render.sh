...
if [ ! -f $DECAPOD_BASE_DIR ]; then
  ...
  git clone -b $BRANCH $DECAPOD_BASE_URL $DECAPOD_BASE_DIR
fi
...
cp -r $DECAPOD_BASE_DIR/$APP_NAME/base $APP_NAME/
...
do
  ...
  docker run -i -v $(pwd)/$APP_NAME:/$APP_NAME --name kustomize-build-$APP_NAME sktdev/decapod-kustomize:v1 kustomize build --enable_alpha_plugins /$APP_NAME/site/$i -o /$APP_NAME/output/$i/$APP_NAME-manifest.yaml
  build_result=$?
